...
//		System.out.println(issues.size());
	...
	private void compareParents(AGNode a, AGNode b, List<String> issues) {
		...
		for (Contains c : ag1.getContains()) {
			if (c.child.toString().equals(a.toString())) {
				c1 = c;
				break;
			}
		}
		for (Contains c : ag2.getContains()) {
			if (c.child.toString().equals(b.toString())) {
				c2 = c;
				break;
			}
		}
	...
	private void compareSiblings(AGNode a, AGNode b, List<String> issues) {
		...
		while (sa.size() > 0) {
			...
			// Adds the matched edge to the list and removes the corresponding edge from the list of siblings
			if (match != null) {
				matched.put(s, match);
				sb.remove(match);
			} else {
		...
		for (Sibling s : unmatch1) {
			DomNode x = s.getNode1();
			DomNode y = s.getNode2();
	...
	private String errMsg(String msg, Edge x, Edge y) {
